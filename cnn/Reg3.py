...
while i < 1:
    ...
    if search_word in spy.read():
        print('This username is unavailable, please try again!')
        Username = input('Please choose a username:').lower()
    else:
        i = 1
else:
    f = open("./Database/usernames.txt", "a")
    f.write("\n" + Username)
    f.close()
...
while cap.isOpened():
    ...
    for i, rect in enumerate(rects):
        ...
        if judge:
            ...
            text = 'Please remove your glasses!'
            textsize = cv2.getTextSize(text, font, 1, 2)
            textX = int((640 - (textsize[0][0])) / 2)
            cv2.putText(img, 'Please remove your glasses!', (textX, 445), font, 1, (255, 255, 255), 2)
        else:
            ...
            font = cv2.FONT_HERSHEY_DUPLEX
            text = 'press "Q" when ready!'
            textsize = cv2.getTextSize(text, font, 1, 2)
            textX = int((640 - (textsize[0][0])) / 2)
            cv2.putText(img, 'press "Q" when ready!', (textX, 445), font, 1, (255, 255, 255), 2)
            k = cv2.waitKey(10)
            j = 30
            if k == ord('q'):
                ...
                while j >= 10:
                    ...
                    if j % 10 == 0:
                        ...
                        font = cv2.FONT_HERSHEY_DUPLEX
                        cv2.putText(img, str(j // 10), (250, 250), font, 10, (255, 255, 255), 10, cv2.LINE_AA)
